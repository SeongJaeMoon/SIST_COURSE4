	...
	$(document).ready(function() {
		...
		$("button.blindON").on("click", function() {
			if (confirm("선택한 게시물을 블라인드 지정할까요?")) {
				location.assign("<%=contextRoot%>/views/admin/adminblind.jsp?gid="+$(this).val()+"&blind=1");
			}
		});
		$("button.blindOFF").on("click", function() {
			if (confirm("선택한 게시물을 블라인드 해제할까요?")) {
				location.assign("<%=contextRoot%>/views/admin/adminblind.jsp?gid="+$(this).val()+"&blind=0");
			}
		});
		//-----------------
		...
		<nav class="navbar navbar-default">
			<div class="container-fluid">
				...
				<ul class="nav navbar-nav">
					<li class="active"><a href="<%=contextRoot%>/views/admin/adminbooklist.jsp">방명록 관리</a></li>
					<li><a href="<%=contextRoot%>/views/admin/adminpicturelist.jsp">사진 관리</a></li>
					<li><a href="<%=contextRoot%>/views/login/logout.jsp">[<%=name_%>/<%=id%>]로그 아웃</a></li>
				</ul>
			</div>
		</nav>

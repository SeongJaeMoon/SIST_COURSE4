...
public class QABoardDAO {
	...
	public List<QABoard> list(){
		...
		String sql = "SELECT qid, title, writeday, content_, clientip, blind, privacy, (SELECT replyContent FROM qaboardreplytable WHERE qid = qa.qid) AS replyContent FROM QABoardTable qa WHERE blind = 0 ORDER BY qid DESC";
	...
	public int add(QABoard qa) {
		...
		String sql2 = "SELECT qid FROM QABoardTable ORDER BY qid DESC LIMIT 1";
	...
	public QABoard pwlist(String qid, String pw){
		
		String sql = "SELECT qid, content_, pw FROM QABoardTable INEER JOIN QABoardPasswordTable USING(qid) WHERE qid = ? AND pw = ?";

...
<script>
	$(document).ready(function() {
		...
		// 첫 페이지 Previous 버튼 비활성화
		 ...
		 if(next > Math.ceil(totalCount/limitCount)){
		       $("button.btnNext").attr("disabled", "disabled");
		   }
		 
		 // 이전, 다음 버튼에 대한 이벤트 처리
		 $("button.btnPrev").on("click", function() {
		  location.assign("${pageContext.request.contextPath}/guestbook/list?pageNum="+prev);
		 });
		 
		 
		 $("button.btnNext").on("click", function() {
		  location.assign("${pageContext.request.contextPath}/guestbook/list?pageNum="+next);
		 });
			...
			<div class="panel-body">
				<div id="myCarousel" class="carousel slide" data-ride="carousel">
					...
					<ol class="carousel-indicators">
						<c:forEach var ="p" items="${picList}" varStatus = "status"> 
							<li data-target="#myCarousel" data-slide-to="${p}" class="${status.first ? 'active' : ''}"></li>
						</c:forEach>	
					</ol>
						<!-- Wrapper for slides -->
